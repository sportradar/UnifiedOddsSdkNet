...
namespace Sportradar.OddsFeed.SDK.Test.Shared
{
    internal class TestDataRouterManager : IDataRouterManager
    {
        ...
        internal TestDataRouterManager(ICacheManager cacheManager, ExceptionHandlingStrategy exceptionHandlingStrategy = ExceptionHandlingStrategy.THROW)
        {
        ...
        private string FindUriReplacement(string path, string cultureIso)
        {
            return FindUriReplacement(path, cultureIso, path);
        }
        ...
        public async Task<IEnumerable<Tuple<URN, URN>>> GetSportEventsForTournamentAsync(URN id, CultureInfo culture, ISportEventCI requester)
        {
            ...
            using (var stream = FileHelper.GetResource(resourceName))
            {
                if (stream != null)
                {
                    ...
                    if (result != null)
                    {
                        await LogSaveDtoAsync(URN.Parse($"sr:sport_events:{result.Items.Count()}"), result, culture, DtoType.SportEventSummaryList, requester)
        ...
        private async Task GetCompetitorProfileAsync(URN id, CultureInfo culture, ISportEventCI requester)
        {
            ...
            Debug.WriteLine($"DRM-GetCompetitorProfileAsync for {id} and culture {culture.TwoLetterISOLanguageName} - START");
            ...
            Debug.WriteLine($"DRM-GetCompetitorProfileAsync for {id} and culture {culture.TwoLetterISOLanguageName} - END");
        }
