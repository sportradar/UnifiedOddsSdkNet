...
namespace Sportradar.OddsFeed.SDK.Test.Shared
{
    public class TestLocalizedNamedValueCache : ILocalizedNamedValueCache
    {
        ...
        public TestLocalizedNamedValueCache(IDictionary<int, IDictionary<CultureInfo, string>> namedValues,
        ...
        public Task<ILocalizedNamedValue> GetAsync(int id, IEnumerable<CultureInfo> cultures = null)
        {
            if (_namedValues.TryGetValue(id, out var values))
            {
                ...
                var localizedNamedValue =
                    new LocalizedNamedValue(id, values.Where(s => wantedCultures.Contains(s.Key)).ToDictionary(d => d.Key, d => d.Value), wantedCultures.First());
