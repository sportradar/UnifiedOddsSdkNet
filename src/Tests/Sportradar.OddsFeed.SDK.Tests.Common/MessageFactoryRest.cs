...
public static class MessageFactoryRest
{
    ...
    {
        id = id == 0 ? SR.I1000 : id;
        return new category { id = SR.Urn(id, "category").ToString(), name = $"Category name {id}", country_code = "en" };
    }

    public static sport GetSport(int id = 0)
    {
        return new sport { id = id == 0 ? SR.Urn("sport", 100).ToString() : SR.Urn(id, "sport").ToString(), name = $"Sport {SR.S1000}" };
    }

    public static currentSeason GetCurrentSeason(int id = 0)
    {
        return new currentSeason { id = id == 0 ? SR.Urn("season", 100000).ToString() : SR.Urn(id, "season").ToString(), name = $"Season name {SR.S1000}" };
    }
                   ...
                   competitor = new teamExtended
                                {
                                    ...
                                    reference_ids = referenceIds
                                                    ?.Select(s => new competitorReferenceIdsReference_id
                                                                  {
                                                                      name = s.Key,
                                                                      value = s.Value
                                                                  })
    ...
    public static coverageInfo GetCoverageInfo(int subItemCount = 0)
    {
        ...
        return new coverageInfo { level = SR.S100, live_coverage = true, coverage = items.ToArray(), covered_from = "tv" };
    }

    public static delayedInfo GetDelayedInfo(int id = 0)
    {
        return new delayedInfo { id = id == 0 ? SR.I1000 : id, description = SR.S1000 };
    }
    ...
    {
        return new desc_specifiersSpecifier { type = "Type " + SR.S100, name = "Name " + SR.S(), description = "Description " + SR.S1000 };
    }
    ...
    public static matchSummaryEndpoint GetMatchSummaryEndpoint(int id = 0, int subItemCount = 2)
    {
        ...
        return new matchSummaryEndpoint { coverage_info = GetCoverageInfo(subItemCount), sport_event = sportEvent, sport_event_conditions = GetSportEventConditions() };
    }

    public static tournamentGroup GetGroup()
    {
        return new tournamentGroup { name = "Group " + SR.S1000 };
    }

    public static info GetInfo()
    {
        return new info { key = SR.S10000P, value = SR.S1000 };
    }

    public static referenceIdsReference_id GetReference()
    {
        return new referenceIdsReference_id { name = SR.S10000P, value = SR.S10000 };
    }
    ...
    {
        return new mappingsMappingMapping_outcome { outcome_id = SR.S10000, product_outcome_id = SR.S1000, product_outcome_name = SR.S1000 };
    }
    ...
    {
        return new referee { id = id == 0 ? SR.Urn("player").ToString() : SR.Urn(id, "player").ToString(), name = "Name " + SR.S1000, nationality = "Nationality " + SR.S100, };
    }
    ...
    public static tournamentGroup GetTournamentGroup(int subItemCount = 0)
    {
        ...
        return new tournamentGroup { name = "Group " + SR.S1000, competitor = GetTeamList(subItemCount).ToArray() };
    }

    public static tournamentLength GetTournamentLength()
    {
        return new tournamentLength { start_date = DateTime.Now, start_dateSpecified = true, end_date = DateTime.Today.AddDays(1), end_dateSpecified = true };
    }
    ...
    {
        return new tvChannel { name = "Name " + SR.S10000P, start_time = DateTime.Now, start_timeSpecified = startTimeSpecified };
    }
    ...
    public static EventType GetEventType(URN eventId, int nbrMarkets = 10)
    {
        ...
        for (var i = 0; i < nbrMarkets; i++)
        {
            ...
            {
                var outcomeType = new OutcomeType { id = textOrNbr ? $"sr:exact_goals:3+:{startNbr + j}" : (startNbr + j).ToString() };
                outcomeTypes.Add(outcomeType);
            }

            var marketType = new MarketType { id = i + 1, specifiers = SR.I100 > 95 ? "total=1" : string.Empty, outcome = outcomeTypes.ToArray() };
            marketTypes.Add(marketType);
        }

        return new EventType { id = eventId.ToString(), markets = marketTypes.ToArray() };
    }
    ...
    public static FilteredEventType GetFilteredEventType(URN eventId, int nbrMarkets = 10)
    {
        ...
        return new FilteredEventType { id = eventId.ToString(), markets = marketTypes.ToArray() };
    }

    public static AvailableSelectionsType GetAvailableSelections(URN eventId, int nbrMarkets = 10)
    {
        return new AvailableSelectionsType { generated_at = DateTime.Now.ToString(SdkInfo.ISO8601_24H_FullFormat), @event = GetEventType(eventId, nbrMarkets) };
    }
        ...
        return new CalculationResponseType
               {
                   ...
                   calculation = new CalculationResultType { odds = SR.D(100), probability = SR.D0 }
               };
    }
        ...
        return new FilteredCalculationResponseType
               {
                   generated_at = DateTime.Now.ToString(SdkInfo.ISO8601_24H_FullFormat),
                   available_selections = eventTypes.ToArray(),
                   calculation = new FilteredCalculationResultType { odds = SR.D(100), probability = SR.D0 }
               };
    }
}