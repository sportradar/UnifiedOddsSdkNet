...
public class MessageFactoryFeed
{
    ...
    {
        return new alive { product = producer.Id, timestamp = GetTimestamp(), subscribed = subscribed ? 1 : 0 };
    }

    public static snapshot_complete GetSnapshotComplete(IProducer producer, long requestId)
    {
        return new snapshot_complete { product = producer.Id, timestamp = GetTimestamp(), request_id = requestId };
    }
    ...
    {
        return new market { id = id, specifiers = specifiers };
    }
