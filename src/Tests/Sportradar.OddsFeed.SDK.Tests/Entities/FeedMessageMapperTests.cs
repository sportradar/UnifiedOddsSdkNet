...
public class FeedMessageMapperTests
{
    ...
    public void ProbabilityWithMarketWithCashoutStatusIsMapped()
    {
        ...
        TestData.FillMessageTimestamp(message);
        message.SportId = URN.Parse("sr:sport:1000");
        var entity = _mapper.MapCashOutProbabilities<ICompetitionV3>(message, new[] { TestData.Culture }, null);
        Assert.NotNull(entity);
        Assert.NotNull(entity.Markets);
        Assert.Equal(1, entity.Markets.Count(w => ((IMarketWithProbabilitiesV2)w).CashoutStatus != null));
        var marketWithCashoutStatus = entity.Markets.First(w => ((IMarketWithProbabilitiesV2)w).CashoutStatus != null);
        Assert.Equal(CashoutStatus.AVAILABLE, ((IMarketWithProbabilitiesV2)marketWithCashoutStatus).CashoutStatus);
    }
