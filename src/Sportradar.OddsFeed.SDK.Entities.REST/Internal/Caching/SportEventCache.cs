...
namespace Sportradar.OddsFeed.SDK.Entities.REST.Internal.Caching
{
    ...
    internal class SportEventCache : SdkCache, ISportEventCache, IHealthStatusProvider, IDisposable, IExportableSdkCache
    {
        ...
        {
            Task.Run(async () => {
                await OnTimerElapsedAsync(sender, e).ConfigureAwait(false);
            });
        }
        ...
        private async Task OnTimerElapsedAsync(object sender, EventArgs e)
        {
        ...
        {
            await OnTimerElapsedAsync(null, null).ConfigureAwait(false); // this is almost not needed anymore, since we call all tournaments for all sports :)
