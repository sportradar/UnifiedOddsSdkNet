...
namespace Sportradar.OddsFeed.SDK.Entities.REST.Internal.Caching
{
    ...
    internal class LocalizedNamedValueCache : ILocalizedNamedValueCache, IHealthStatusProvider, IDisposable
    {
        ...
        public LocalizedNamedValueCache(IDataProvider<EntityList<NamedValueDTO>> dataProvider, ICollection<CultureInfo> cultures, ExceptionHandlingStrategy exceptionStrategy, string cacheName)
        {
        ...
        private async Task FetchAndMerge(CultureInfo culture)
        {
            ...
            Metric.Context("CACHE").Meter($"LocalizedNamedValueCache-{_cacheName}->FetchAndMerge", Unit.Calls);
            ...
            CacheLog.Info($"{_cacheName}: {record.Items.Count()} items retrieved for locale '{culture.TwoLetterISOLanguageName}'.");
        }
        ...
        private async Task GetInternalAsync(IEnumerable<CultureInfo> cultures)
        {
            ...
            {
                ExecutionLog.Warn("An exception occurred while attempting to retrieve match statuses.", ex);
            }
