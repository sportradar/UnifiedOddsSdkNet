        ...
        private void OnRawFeedMessageReceived(object sender, RawFeedMessageEventArgs e)
        {
            ...
            using (var t = timer.NewContext($"{e.MessageInterest} - {e.FeedMessage?.EventId}"))
            {
                try
                {
                    ...
                    Log.Info($"Dispatching raw feed message [{e.MessageInterest}]: {e.FeedMessage?.GetType().Name} for event {e.FeedMessage?.EventId}{requestId} took {t.Elapsed.TotalMilliseconds} ms.");
                }
                catch (Exception ex)
                {
