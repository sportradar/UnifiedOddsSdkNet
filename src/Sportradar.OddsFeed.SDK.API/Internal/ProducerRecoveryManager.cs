    ...
    internal class ProducerRecoveryManager : IProducerRecoveryManager
    {
        ...
        public void ConnectionShutdown()
        {
            ...
            lock (_syncLock)
            {
                ...
                {
                    ExecutionLog.Warn($"Connection shutdown detected. Producer={Producer}, Status={Enum.GetName(typeof(ProducerRecoveryStatus), Status)}, Action: Resetting current recovery operation.");
        ...
        public void ConnectionRecovered()
        {
            ...
            lock (_syncLock)
            {
                ...
                {
                    ExecutionLog.Warn($"Connection recovery detected after previous recovery request started. Producer={Producer}, Status={Enum.GetName(typeof(ProducerRecoveryStatus), Status)}, Action: Resetting current recovery operation.");
                    _recoveryOperation.Reset();
                    newStatus = ProducerRecoveryStatus.Error;
                }
        ...
        public void ProcessUserMessage(FeedMessage message, MessageInterest interest)
        {
            ...
            {
                ExecutionLog.Error($"The producer ({message.ProducerId}) of the message and the Producer {Producer.Name} ({Producer.Id}) associated with this manager do not match.");
                return;
            }
            ...
            lock (_syncLock)
            {
                ...
                {
                    ExecutionLog.Error($"An unexpected exception occurred while processing user message. Producer={_producer.Id}, Session={interest.Name}, Message={message}", ex);
                }
        ...
        public void ProcessSystemMessage(FeedMessage message)
        {
            ...
            lock (_syncLock)
            {
                ...
                {
                    ExecutionLog.Error($"An unexpected exception occurred while processing system message. Producer={_producer.Id}, message={message}", ex);
                }
